...
- **Два режима источника данных**:
  ...
  - встроенный **локальный клиент на собственном BitTorrent-движке (custom engine)** без отдельного TorrServer.
...
4. Откройте **Настройки** и выберите режим работы:
    - **Локальный клиент (custom engine)** — если хотите работать без внешнего TorrServer.
...
| `data_mode` | `local_client` для встроенного custom engine-клиента или `torrserver` для внешнего TorrServer. |
...
- собственный BitTorrent-движок (custom engine) уже включён в исходники; внешний libtorrent больше не требуется;
...
- `source/datasource/` — абстракции удалённого TorrServer и локального custom engine-бэкенда.
...
- `docs/` — технические заметки по custom engine и стабильности загрузок.

## Текущий статус

Основной фокус проекта — стабильная потоковая загрузка и удобный интерфейс для Nintendo Switch. В кодовой базе уже есть локальный custom engine-бэкенд с параллельным скачиванием блоков, µTP, DHT и планировщиком чтения частей, мониторинг зависаний, настройки режима работы и веб-добавление раздач. Отдельные сценарии установки/стриминга всё ещё требуют тестирования на реальном устройстве и могут меняться.

## Лицензия

Проект распространяется под лицензией GPLv3 или более поздней версии.
