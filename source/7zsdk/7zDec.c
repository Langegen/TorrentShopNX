...
static SRes CheckSupportedFolder(const CSzFolder *f)
{
  ...
  /* BCJ2 layout produced by 7-Zip (and forks): 2 coders [main][BCJ2(4 streams)] */
  if (f->NumCoders == 2 && IS_BCJ2(&f->Coders[1]))
  {
    ...
    Byte *tempBuf[])
{
  ...
  for (ci = 0; ci < folder->NumCoders; ci++)
  {
    ...
    if (IS_MAIN_METHOD((UInt32)coder->MethodID))
    {
      ...
      else if (folder->NumCoders == 2 && IS_BCJ2(&folder->Coders[1]))
      {
        /* BCJ2 2-coder form: the main stream is decoded into the tail
           of outBuffer; BCJ2 then consumes it plus 3 pack sub-streams. */
        const UInt64 mainUnpack = unpackSizes[ci]; /* ci == 0 */
        if (mainUnpack > outSize) /* check it */
          return SZ_ERROR_PARAM;
        outBufCur = outBuffer + (outSize - (SizeT)mainUnpack);
        outSizeCur = (SizeT)mainUnpack;
      }
    ...
    else if (coder->MethodID == k_BCJ2)
    {
      ...
      else if (ci == 1 && folder->NumCoders == 2)
      {
        /* BCJ2 2-coder form: main stream (already decoded into the tail of
           outBuffer by coder0) + 3 pack sub-streams (CALL, JUMP, RC). */
        UInt32 k;
        const UInt64 mainUnpack = unpackSizes[0];
        Byte *mainBuf = outBuffer + (outSize - (SizeT)mainUnpack);

        for (k = 0; k < 3; k++)
        {
          /* packPositions here are folder-local and ordered by pack-stream
             order: [0]=main, then CALL, JUMP, RC. */
          const UInt64 offs = packPositions[1 + k];
          const UInt64 subSize = packPositions[2 + k] - offs;
          tempSizes[k] = (SizeT)subSize;
          if (tempSizes[k] != subSize)
            return SZ_ERROR_MEM;
          tempBuf[k] = (Byte *)ISzAlloc_Alloc(allocMain, tempSizes[k]);
          if (!tempBuf[k] && tempSizes[k] != 0)
            return SZ_ERROR_MEM;
          RINOK(LookInStream_SeekTo(inStream, startPos + offs))
          RINOK(SzDecodeCopy(subSize, inStream, tempBuf[k]))
        }
...

        {
          ...
          p.bufs[0] = mainBuf;        p.lims[0] = mainBuf + (SizeT)mainUnpack;
          p.bufs[1] = tempBuf[0];     p.lims[1] = tempBuf[0] + tempSizes[0];
          p.bufs[2] = tempBuf[1];     p.lims[2] = tempBuf[1] + tempSizes[1];
          p.bufs[3] = tempBuf[2];     p.lims[3] = tempBuf[2] + tempSizes[2];
