...
void Label::setText(const std::string& text)
{
    ...
    if (trans && OPENCC_ON)
    {
        convertedText = Label::STConverter(text);
    }
#endif

    this->truncatedText = convertedText;
    this->fullText      = convertedText;
    this->stringLength  = strLen(convertedText);
    ...
    if (vg)
    {
        ...
        float bounds[4];
        nvgTextBounds(vg, 0, 0, ELLIPSIS, nullptr, bounds);
        this->ellipsisWidth = bounds[2] - bounds[0];

        nvgTextBounds(vg, 0, 0, this->fullText.c_str(), nullptr, bounds);
        this->requiredWidth = bounds[2] - bounds[0] - 0.5f;

        float width = this->getWidth();
        if (width > 0 && width < this->requiredWidth && !this->isWrapping)
        {
...
void Label::resetScrollingAnimation()
{
    ...
    if (this->animated && width < this->requiredWidth)
    {
        // Only restart if the text or width actually changed
        if (this->animating && this->lastAnimatedText == this->fullText && this->lastAnimatedWidth == width)
            return;

        this->stopScrollingAnimation();

        this->lastAnimatedText = this->fullText;
        this->lastAnimatedWidth = width;
    ...
    {
        this->stopScrollingAnimation();
        this->lastAnimatedText = "";
        this->lastAnimatedWidth = -1.0f;
    }
}
